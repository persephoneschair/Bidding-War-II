...
public class AudioManager : MonoBehaviour
{
    ...
    public static AudioManager GetAudioManager { get; private set; }
    private void Awake()
    {
        if (GetAudioManager != null && GetAudioManager != this)
            Destroy(this);
        else
    ...
    public AudioSource playOnceSource;
    public AudioSource loopingSource;

    public bool playedUnique;
    ...
    public void Play(OneShotClip oneShot, float delay = 0f)
    {
        if (delay != 0f)
            ...
            playOnceSource.PlayOneShot(stings[(int)oneShot]);
    }
