...
public class GameplayPennys : MonoBehaviour
{
    ...
    {
        List<PlayerObject> list = HostManager.GetHost.players.OrderByDescending(p => p.totalCorrect).ThenBy(p => p.twitchName).Where(x => x.points > 0).ToList();
        PlayerPennyData ppd;
    ...
    {
        List<PlayerObject> topTwo = HostManager.GetHost.players.Where(x => !x.eliminated).OrderByDescending(x => x.points).ToList();
        LoadMedalJSON();
        ...
        List<PlayerObject> lobbyOrdered = HostManager.GetHost.players.Where(x => x.eliminated).OrderByDescending(x => x.totalCorrect).ThenByDescending(x => x.maxPoints).ToList();
        foreach(PlayerObject player in lobbyOrdered.Where(x => x.totalCorrect == lobbyOrdered[0].totalCorrect && x.maxPoints == lobbyOrdered[0].maxPoints))
    ...
    {
        string pennyPath = Operator.GetOperator.testMode ? path + @"\NewPennysTest.txt" : path + @"\NewPennys.txt";
        string medalPath = Operator.GetOperator.testMode ? path + $@"\{gameName}Test.txt" : path + $@"\{gameName}.txt";
